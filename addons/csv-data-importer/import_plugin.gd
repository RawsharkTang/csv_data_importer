...
func _import(source_file, save_path, options, platform_variants, gen_files):
	...
	var str = save_path.split("/")
	str = str[-1].split(".")
	# do not setup here
	var data := preload("csv_data.gd").new(false)

	# check if already imported
	var pre_imported = load(filename)
	if pre_imported:
		data = pre_imported
		print("updating pre-imported " + data.resource_name + ".csv")
		data._initialed = false  ## reset initialization
		pass

	var rows = lines.get_data()
	data.records = rows
	data.headers = meta.headers
	data.resource_name = str[0]
...
func _parse_typed_value(p_value: String, p_type: String):
	match p_type:
		...
		"bool":
			...
			return str_to_var(p_value.to_lower())
		"json":
			if p_value.is_empty():
				p_value = "[]"
